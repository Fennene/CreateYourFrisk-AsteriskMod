...
public class SelectMessage : TextMessage {
    public SelectMessage(IList<string> options, bool singleList, IList<string> colorPrefixes = null) : base("", false, true) {
        ...
        string prefix = AsteriskEngine.AsteriskChar + " ";              // Prefix used for new lines
        // --------------------------------------------------------------------------------
        ...
        for (int i = 0; i < options.Count; i++) {
            ...
            if (options[i] == null || options[i] == "" || options[i].Contains("PAGE "))
                ...
                if (options[i-1] == null || options[i-1] == "")
                    ...
                    // --------------------------------------------------------------------------------
                    //prefix = "* ";
                    prefix = AsteriskEngine.AsteriskChar + " ";
                    // --------------------------------------------------------------------------------
            if (options[i] != null)
                // --------------------------------------------------------------------------------
                //                          Asterisk Mod Modification
                ...
                options[i] = options[i].TrimStart(AsteriskEngine.AsteriskChar, ' ');
                // --------------------------------------------------------------------------------
