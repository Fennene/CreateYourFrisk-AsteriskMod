...
function StateStarting(oldState) -- requited
	...
	for i = 1, #items do
		squares[i] = CreateSprite("FakeUI/spr_itemmenu_square", "BelowBullet")
		local absy = 160 + (((#items - 1) / 2) * 10) - ((i - 1) * 10)
		squares[i].MoveToAbs(590, absy)
	end
	arrows[1] = CreateSprite("FakeUI/spr_itemmenu_arrow", "BelowBullet")
	arrows[1].MoveToAbs(590, 160 + 10 + (((#items - 1) / 2)) * 10)
	arrows[1]["init_y"] = arrows[1].absy
	arrows[1].alpha = 0
	arrows[2] = CreateSprite("FakeUI/spr_itemmenu_arrow", "BelowBullet")
